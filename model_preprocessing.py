...
plt.hist(
    cancer_df_scaled.loc[cancer_df_scaled["target"] == 0, "worst concave points"],
    bins,
    alpha=0.5,
    label="Злокачественная",
)

# то же самое, но target == 1 (доброкачественная)
plt.hist(
    cancer_df_scaled.loc[cancer_df_scaled["target"] == 1, "worst concave points"],
    bins,
    alpha=0.5,
    label="Доброкачественная",
)
